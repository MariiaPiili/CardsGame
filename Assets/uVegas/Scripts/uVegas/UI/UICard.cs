...
namespace uVegas.UI
{
    ...
    /// </summary>
    public class UICard : MonoBehaviour, IComparable<UICard>
    {
        public Card currentCard; // The card currently displayed
        ...
        private void OnValidate()
        {
            ...
            {
                layoutGroup = transform.parent.parent.GetComponent<GridLayoutGroup>();
            }
            if(layoutGroup != null )
            {
                recTransform.sizeDelta = layoutGroup.cellSize;
            }
        }
        public void Update()
        {
        ...
        public void Init(Card card, CardTheme theme, bool isHidden = false)
        {
            currentCard = card;
            currentTheme = theme;
        ...
        {
            if ((((int)currentCard.suit * 10) + (int)currentCard.rank) < (((int)other.currentCard.suit * 10) + (int)other.currentCard.rank))
            {
                return -1;
            }
            else if ((((int)currentCard.suit * 10) + (int)currentCard.rank) > (((int)other.currentCard.suit * 10) + (int)other.currentCard.rank))
            {
                return 1;
            }
            else
